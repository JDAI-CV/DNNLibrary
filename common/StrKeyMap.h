...
class StrKeyMap {
   ...
   public:
    ...
    const typename map_t::const_iterator begin() const { return map_.begin(); }
    const typename map_t::const_iterator end() const { return map_.end(); }
    void clear() { map_.clear(); }
    bool has(const std::string &key) { return map_.find(key) != map_.end(); }

    void insert(const std::pair<std::string, V> &p) { map_.insert(p); }
};

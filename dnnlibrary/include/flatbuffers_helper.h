...
#define UNPACK(name)    \
    const auto name = unpack_fbs(param->name());    \
...
#define ADD_LAYER(name, shape_func, ...) \
    ...
    if (mat_map_.find(LAST_ARG(__VA_ARGS__)) == mat_map_.end()) {  \
        ...
        add_mat(LAST_ARG(__VA_ARGS__), std::make_shared<Mat>(output_shape[1], output_shape[2], output_shape[3], input_mat.data_type, LAST_ARG(__VA_ARGS__)));    \
    }   \
...
inline std::vector<std::string> unpack_fbs(const flatbuffers::Vector<flatbuffers::Offset<flatbuffers::String>> *fbs_vec) {
...
inline std::vector<int32_t> unpack_fbs(const flatbuffers::Vector<int32_t> *fbs_vec) {
    using fbsoff_t = flatbuffers::uoffset_t;

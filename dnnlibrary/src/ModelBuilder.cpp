...
ModelBuilder::Index ModelBuilder::AddDepthWiseConv(
    ...
    if (!bias_name.has_value()) {
        Shape weightDimen =
            shaper_[weight_name];  // 1, height, width, num_output
        ...
        if (weight_type == Type::TENSOR_FLOAT32) {
            biasIndexValue = FillOperand(
                bias_name_val, {Type::TENSOR_FLOAT32, bias_dims}, 0.f);
        } else if (weight_type == Type::TENSOR_QUANT8_ASYMM) {
            const auto input_scale =
                operand_types_.at(input_name).operandType.scale;
            const auto weight_scale =
                operand_types_.at(weight_name).operandType.scale;
            biasIndexValue = FillOperand(
                bias_name_val,
    ...
    shaper_.DepthwiseConv(input_name, strideX, strideY, 1, 1, paddingLeft,
                          paddingRight, paddingTop, paddingBottom, weight_name,
                          output_name);
    if (bias_name.has_value() && operand_types_.at(input_name).isQuant()) {
        ...
        const auto weight_scale =
            operand_types_.at(weight_name).operandType.scale;
        const auto bias_scale =
            operand_types_.at(bias_name_val).operandType.scale;
    ...
    AddScalarOperands(input_indexes, paddingLeft, paddingRight, paddingTop,
                      paddingBottom, strideX, strideY, depthMultiplier,
                      activation);
    DNN_ASSERT(
        (operand_types_.at(input_name).type == Type::TENSOR_FLOAT32 &&
         operand_types_.at(weight_name).type == Type::TENSOR_FLOAT32) ||
            (operand_types_.at(input_name).type == Type::TENSOR_QUANT8_ASYMM &&
             operand_types_.at(weight_name).type == Type::TENSOR_QUANT8_ASYMM),
        "");
    OperandType operand_type =
        GetOperandType(operand_types_.at(input_name).type, shaper_[output_name],
                       output_quant_info);
    const auto output_index = AddOperation(ANEURALNETWORKS_DEPTHWISE_CONV_2D,
...
ModelBuilder::Index ModelBuilder::AddConv(
    ...
    if (!bias_name.has_value()) {
        Shape weightDimen =
            shaper_[weight_name];  // num_output, height, width, num_input
    ...
    shaper_.Conv(input_name, strideX, strideY, 1, 1, paddingLeft, paddingRight,
                 paddingTop, paddingBottom, weight_name, output_name);
    if (operand_types_.at(input_name).isQuant()) {
        const auto input_scale =
            operand_types_.at(input_name).operandType.scale;
        const auto weight_scale =
            operand_types_.at(weight_name).operandType.scale;
    ...
    IndexSeq input_indexes{input, weight, biasIndexValue};
    AddScalarOperands(input_indexes, paddingLeft, paddingRight, paddingTop,
                      paddingBottom, strideX, strideY, activation);
    DNN_ASSERT(
        (operand_types_.at(input_name).type == Type::TENSOR_FLOAT32 &&
         operand_types_.at(weight_name).type == Type::TENSOR_FLOAT32) ||
            (operand_types_.at(input_name).type == Type::TENSOR_QUANT8_ASYMM &&
...
ModelBuilder::Index ModelBuilder::AddStridedSlice(
    const string &input_name, const vector<int32_t> &starts,
    const vector<int32_t> &ends, const vector<int32_t> &strides,
    int32_t beginMask, int32_t endMask, int32_t shrinkAxisMask,
    const string &output_name) {
    const auto input = operand_indexes_[input_name];

    const auto startsIndex = AddTensorFromBuffer(
        output_name + "_starts", &starts[0],
        {Type::TENSOR_INT32, Shape{static_cast<uint32_t>(starts.size())}});
    const auto endsIndex = AddTensorFromBuffer(
        output_name + "_ends", &ends[0],
        {Type::TENSOR_INT32, Shape{static_cast<uint32_t>(ends.size())}});
    const auto stridesIndex = AddTensorFromBuffer(
        output_name + "_strides", &strides[0],
        {Type::TENSOR_INT32, Shape{static_cast<uint32_t>(strides.size())}});

    shaper_.StridedSlice(input_name, starts, ends, strides, beginMask, endMask,
                         shrinkAxisMask, output_name);
    ...
    const OperandType operand_type(operand_types_.at(input_name).type,
                                   shaper_[output_name]);
    const auto output_index = AddOperation(ANEURALNETWORKS_STRIDED_SLICE,
                                           input_indexes, operand_type)[0];
    RegisterOperand(output_name, output_index, operand_type);
    return output_index;
}
...
ModelBuilder::Index ModelBuilder::AddBatchToSpaceND(
    ...
    const OperandType operand_type(operand_types_.at(input_name).type,
                                   shaper_[output_name]);
    const auto output_index = AddOperation(ANEURALNETWORKS_BATCH_TO_SPACE_ND,
                                           input_indexes, operand_type)[0];
    RegisterOperand(output_name, output_index, operand_type);
    return output_index;
}
...
ModelBuilder::Index ModelBuilder::AddPool(
    ...
    shaper_.Pool(input_name, strideX, strideY, paddingLeft, paddingRight,
                 paddingTop, paddingBottom, height, width, output_name);
    ...
    if (poolingType == PoolingType::MAX_POOL) {
        output_index = AddOperation(ANEURALNETWORKS_MAX_POOL_2D, input_indexes,
                                    operand_type)[0];
    } else if (poolingType == PoolingType::AVE_POOL) {
        output_index = AddOperation(ANEURALNETWORKS_AVERAGE_POOL_2D,
                                    input_indexes, operand_type)[0];
    } else {
...
ModelBuilder::Index ModelBuilder::AddReLU(const string &input_name,
                                          const string &output_name) {
    const auto input = operand_indexes_[input_name];
...
ModelBuilder::Index ModelBuilder::AddConcat(const vector<string> &input_names,
                                            int32_t axis,
                                            const string &output_name) {
    IndexSeq inputs;
    ...
    const OperandType operand_type(input_op_type.type, shaper_[output_name],
                                   input_op_type.operandType.scale,
                                   input_op_type.operandType.zeroPoint);
    const auto output_index = AddOperation(ANEURALNETWORKS_CONCATENATION,
                                           input_indexes, operand_type)[0];
    RegisterOperand(output_name, output_index, operand_type);
    return output_index;
}

ModelBuilder::Index ModelBuilder::AddLRN(const string &input_name,
...
ModelBuilder::Index ModelBuilder::AddFC(
    ...
    if (!bias_name.has_value()) {
        ...
        biasIndexValue = FillOperand(weight_name + "_b",
                                     {Type::TENSOR_FLOAT32, bias_dims}, 0.f);
    } else {
...
ModelBuilder::Index ModelBuilder::AddOperationAdd(const string &input_name,
    ...
    const auto input = operand_indexes_[input_name];
    Index scalarIndex =
        FillOperand(output_name + "_add", {Type::TENSOR_FLOAT32, {1}}, scalar);
    IndexSeq inputOperands{input, scalarIndex,
                           OperandFromScalar(ModelBuilder::ACTIVATION_NONE)};
    shaper_.Eltwise(input_name, output_name);
    const OperandType operand_type(operand_types_.at(input_name).type,
                                   shaper_[output_name]);
    const auto output_index =
        AddOperation(ANEURALNETWORKS_ADD, inputOperands, operand_type)[0];
    RegisterOperand(output_name, output_index, operand_type);
    return output_index;
}
...
ModelBuilder::Index ModelBuilder::AddOperationAdd(
    ...
    const OperandType operand_type =
        GetOperandType(operand_types_.at(input1_name).type,
                       shaper_[output_name], output_quant_info);
    const auto output_idx =
...
ModelBuilder::Index ModelBuilder::AddMul(const string &input_name, float scalar,
    ...
    const auto input = operand_indexes_[input_name];
    Index scalarIndex =
        FillOperand(output_name + "_mul", {Type::TENSOR_FLOAT32, {1}}, scalar);
    IndexSeq inputOperands{input, scalarIndex,
                           OperandFromScalar(ModelBuilder::ACTIVATION_NONE)};
    ...
    const OperandType operand_type(operand_types_.at(input_name).type,
                                   shaper_[output_name]);
    const auto output_index =
        AddOperation(ANEURALNETWORKS_MUL, inputOperands, operand_type)[0];
    RegisterOperand(output_name, output_index, operand_type);
    return output_index;
}
...
void ModelBuilder::SetBasePtr(uint8_t *data) { dnn_model_->data_ = data; }

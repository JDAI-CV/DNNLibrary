...
class optional {
   ...
   public:
    ...
    // 4 (C++98 and later) - non-explicit converting copy-construct from
    // optional
    template <typename U>
    optional(
...
template <typename T>
inline optional_constexpr bool operator==(nullopt_t, optional<T> const &x)
    optional_noexcept {
    return (!x);
}
...
template <typename T>
inline optional_constexpr bool operator<=(nullopt_t, optional<T> const &)
    optional_noexcept {
    return true;
}
